...
### Balance and quota visibility

Each card has a **Hide** action. **Manage visibility** remains available above the cards, including when all cards are hidden; use its checkboxes to restore individual items or **Show all** to reset all hidden preferences. Choices are stored in this browser's `localStorage` under `token-monitor-hidden-limits-v1`. They never change Hub records, collection, uploads, or other browsers. If browser storage is blocked, the current page still works and reports that choices cannot persist.

Preferences follow provider/account identity, not list position, changing balances, plan names or status. Snapshots retain only a derived display fingerprint, not the original account key or label. Unidentified same-provider rows are grouped because their order cannot safely distinguish accounts. Older snapshots without fingerprints cannot match identified live accounts until refreshed.
