...
describe('PWA assets', () => {
  ...
  it('discovers and precaches Vite hashed assets during install', async () => {
    ...
    await pending;
    const staticCache = harness.stores.get('token-monitor-static-v23');
...
it.each(['match', 'open', 'put'])('serves a successful static response when cache %s fails', async failure => {
  ...
  if (failure === 'put') (await harness.caches.open('token-monitor-static-v23')).put.mockRejectedValue(new Error('full'));
