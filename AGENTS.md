...
The Node Hub persists high-frequency device ingest with leading+trailing throttling (`TOKEN_MONITOR_HUB_PERSIST_INTERVAL_MS`; `0` restores the legacy write on every ingest). Subscription edits and device deletion force an immediate combined flush so pending device updates become durable with them.
